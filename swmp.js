...
class swmp {
  ...
  prepareWindow() {
    ...
    if (this.type == 'video' || this.type == 'youtube') {
      ...
      this.windowMaximize.addEventListener('click', (event) => {
        event.preventDefault();
        if (this.container.classList.contains('swmp-maximized') ) {
          this.container.classList.remove('swmp-maximized');
        } else {
          this.container.classList.add('swmp-maximized');
        }
      });
    ...
    // Maximize
    this.windowTitlebar.addEventListener('dblclick', (event) => {
